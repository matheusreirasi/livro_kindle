...
function updateTable (clientes) {
    ...
    for (let cliente of clientes) {
        linha += `<tr> <td>${cliente.nome}</td> <td>${cliente.idade} </td> <td>${cliente.uf} <td/> <input type= 'button' class='btn btn-danger' value='X' data-id=${cliente._id} /> </tr>`
    }
    ...
    for (let btn of buttons) {
        ...
        btn.onclick = (evt) => {
            if (confirm("Tem certeza que deseja excluir esse cliente?")) {
                ...
                .then( result => {
                    exibirMensagem("#alertListagem", "<strong>Sucesso!<strong/> Cliente excluído.")
    ...
    */
    ...
    frmCadastro.onsubmit = (evt) => {
        ...
        if (!document.querySelector("input[name='nome']").value) {
            exibirMensagem("#alertCadastro", "O campo nome é obrigatório.")
            return false
        }

        const data = new FormData(frmCadastro) //data possui a chave/valor para ser salvo no mongodb
        ...
        .catch(err => exibirMensagem("#alertCadastro", `Ocorreu um erro: ${err}`))
    }
})
